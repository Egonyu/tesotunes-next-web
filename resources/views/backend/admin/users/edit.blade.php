...
@section('content')
    <div class="card">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data" class="p-6">
            ...
            <div class="mb-6 border-b border-slate-200 pb-6 dark:border-navy-500">
                <h4 class="text-lg font-medium text-slate-700 dark:text-navy-100 mb-4">Profile Picture</h4>
                <div class="flex items-start space-x-6">
                        ...
                        <div class="relative">
                            @if($user->avatar)
                                <img src="{{ asset('storage/' . $user->avatar) }}"
                                     ...
                                     class="h-24 w-24 rounded-full object-cover border-2 border-slate-200 dark:border-navy-500"
                                     id="profile-preview"
                                     onerror="this.src='{{ asset('images/200x200.png') }}'">
                            @else
                                <img src="{{ asset('images/200x200.png') }}"
                                     alt="{{ $user->name }}'s profile picture"
                                     class="h-24 w-24 rounded-full object-cover border-2 border-slate-200 dark:border-navy-500"
                                     id="profile-preview">
                            @endif
                    ...
                    <div class="flex-1">
                        <div class="mt-2">
                            <label class="block text-sm font-medium text-slate-600 dark:text-navy-100 mb-2">Upload New Picture</label>
                            <div class="flex items-center space-x-3">
                                <input type="file"
                                       name="avatar"
                                       id="avatar"
                                       accept="image/*"
                                       class="form-input file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:bg-primary file:text-white hover:file:bg-primary-focus"
                                ...
                                @if($user->avatar)
                                    <label class="inline-flex items-center">
                                        ...
                                        <span class="ml-2 text-xs text-error">Remove current picture</span>
                                    </label>
                                @endif
                            </div>
                            <p class="mt-1 text-xs text-slate-400">JPG, PNG, GIF up to 5MB. Recommended size: 400x400px</p>
                            @error('avatar')
...
function previewImage(event) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            preview.innerHTML = `<img src="${e.target.result}" alt="Profile preview" class="h-24 w-24 rounded-full object-cover border-2 border-slate-200 dark:border-navy-500">`;
        };
