...
export function useRecordPlay() {
    return useMutation({
        mutationFn: (data: {
            ...
            apiPost<{ credits_earned?: number; message?: string }>(
                `/player/record-play`,
                data,
            ),
        onSuccess: (res) => {
            if (res?.credits_earned && res.credits_earned > 0) {
                toast.success(
                    `+${res.credits_earned} credits earned for listening!`,
                    {
                        duration: 3000,
                        icon: "🎵",
                    },
...
export function useArtists(params?: { page?: number; limit?: number }) {
    return useQuery({
        ...
        queryFn: () =>
            apiGet<PaginatedResponse<Artist>>("/artists", { params }),
        retry: 1,
    });
}
...
export function useClaimableArtistsSearch(query: string) {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<PaginatedResponse<Artist>>(
                "/catalog/claimable-artists",
                {
                    params: {
                        claimable_only: 1,
                        per_page: 24,
...
export function useSubmitCatalogClaim() {
    return useMutation({
        mutationFn: (data: {
            ...
            apiPost<{ message?: string; data?: unknown }>(
...
export function useArtistSongs(
    ...
    return useQuery({
        ...
        queryFn: () =>
            apiGet<PaginatedResponse<Song>>(`/artists/${artistId}/songs`, {
...
export function usePublicArtistAlbums(
    artistId: number,
    ...
    return useQuery({
        ...
        queryFn: () =>
            apiGet<PaginatedResponse<Album>>(`/artists/${artistId}/albums`),
    ...
    return useQuery({
        ...
        queryFn: () =>
            apiGet<PaginatedResponse<Artist>>("/artists", {
                params: { limit, sort: "-followers_count" },
            }),
...
export function useFollowArtist() {
    ...
    return useMutation({
        mutationFn: (artistId: number) =>
            apiPost(`/artists/${artistId}/follow`),
...
export function useUnfollowArtist() {
    ...
    return useMutation({
        mutationFn: (artistId: number) =>
            apiPost(`/artists/${artistId}/follow`), // Toggle endpoint
        onSuccess: (_, artistId) => {
            queryClient.invalidateQueries({ queryKey: ["artist", artistId] });
        },
    });
...
export function useGenreSongs(
    ...
    return useQuery({
        ...
        queryFn: () =>
            apiGet<PaginatedResponse<Song>>(`/genres/${genreId}/songs`, {
                params,
            }),
        enabled: !!genreId,
    });
...
export function usePlaylists(params?: { page?: number; limit?: number }) {
    return useQuery({
        ...
        queryFn: () =>
            apiGet<PaginatedResponse<Playlist>>("/playlists", { params }),
    });
}
...
export function usePlaylist(idOrSlug: string | number) {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<{ data: Playlist }>(
                `/playlists/${idOrSlug}`,
            );
            return res.data;
        },
...
export function useUpdatePlaylist() {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            const res =
                data instanceof FormData
                    ? await apiPostForm<{ data: Playlist }>(
                          `/playlists/${playlistId}`,
                          data,
                      )
                    : await apiPut<{ data: Playlist }>(
                          `/playlists/${playlistId}`,
                          data,
                      );
            return res.data;
...
export function useDeletePlaylist() {
    ...
    return useMutation({
        mutationFn: (playlistId: number | string) =>
...
export function useRemovePlaylistArtwork() {
    ...
    return useMutation({
        mutationFn: (playlistId: number | string) =>
            apiDelete(`/playlists/${playlistId}/artwork`),
        onSuccess: (_, playlistId) => {
            queryClient.invalidateQueries({
                queryKey: ["playlist", playlistId],
            });
...
export function useSuggestedPlaylistSongs(
    playlistId: number | string,
    ...
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<{ data: Song[] }>(
                `/playlists/${playlistId}/suggested-songs`,
                {
...
export function useGeneratePlaylistInviteLink() {
    return useMutation({
        mutationFn: ({
            ...
            }>(`/playlists/${playlistId}/invite-link`, {
...
export function useJoinPlaylistInvite() {
    ...
    return useMutation({
        mutationFn: (token: string) =>
            apiPost(`/playlists/invites/${token}/join`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["playlists", "user"] });
            queryClient.invalidateQueries({
                queryKey: ["library", "playlists"],
...
export function usePlaylistInvitePreview(
    token: string,
    options?: { enabled?: boolean },
) {
    return useQuery({
...
export function usePlaylistCollaborators(
    playlistId: number | string,
    options?: { enabled?: boolean },
) {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<{ data: PlaylistCollaborator[] }>(
...
export function useSearch(
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            }>("/v1/public/search", { params: { q: normalizedQuery, type } });
...
export function useLikedSongs(params?: { page?: number; limit?: number }) {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<{ data: { liked_songs: Song[] } }>(
                "/user/library",
            );
...
export function useRecentlyPlayed(limit = 10) {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<{ data: Song[] }>("/history", {
                params: { limit },
            });
            return res.data;
        },
...
export function useFollowedArtists() {
    return useQuery({
        ...
        queryFn: async () => {
            const res = await apiGet<{ data: { followed_artists: Artist[] } }>(
                "/user/library",
            );
...
export function useCheckPurchase(songId: number) {
    return useQuery({
        ...
        queryFn: () =>
            apiGet<{ data: { purchased: boolean } }>(
                `/songs/${songId}/purchase-status`,
            ).then((res) => res.data.purchased),
        enabled: songId > 0,
        staleTime: 5 * 60 * 1000,
    });
}
...
export function usePurchaseSong() {
    ...
    return useMutation({
        mutationFn: ({
            ...
            songId: number;
            payload?: PurchaseSongPayload;
        }) => apiPost<PurchaseResponse>(`/songs/${songId}/purchase`, payload),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({
                queryKey: ["song", "purchase", variables.songId],
            });
...
export function useSongPurchasePaymentStatus(
    songId: number,
    ...
    return useQuery({
        ...
        queryFn: () =>
            apiGet<SongPurchasePaymentStatusResponse>(
                `/songs/${songId}/purchase/payment-status/${reference}`,
            ),
