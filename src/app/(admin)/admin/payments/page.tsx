...
function StatCard({
    ...
    const toneClass = {
        default:
            "border-border/70 bg-gradient-to-br from-background via-background to-muted/40 text-foreground",
        success:
            "border-emerald-200/70 bg-gradient-to-br from-emerald-50 via-background to-emerald-100/70 text-emerald-950 dark:border-emerald-900/70 dark:from-emerald-950/60 dark:via-background dark:to-emerald-900/30 dark:text-emerald-100",
        warning:
            "border-amber-200/70 bg-gradient-to-br from-amber-50 via-background to-amber-100/70 text-amber-950 dark:border-amber-900/70 dark:from-amber-950/50 dark:via-background dark:to-amber-900/30 dark:text-amber-100",
    ...
    return (
        <div
            className={cn(
                "rounded-2xl border p-5 shadow-sm shadow-black/5",
                toneClass,
            )}
        >
            ...
            {detail ? (
                <p className="mt-2 text-sm text-muted-foreground dark:text-current/75">
                    {detail}
                </p>
            ) : null}
        </div>
...
export default function AdminPaymentsPage() {
    ...
    const paymentsQuery = useQuery({
        ...
        queryFn: () =>
            apiGet<{ data: PaymentRow[]; meta: PaginationMeta }>(
                "/admin/payments",
    ...
    const issuesQuery = useQuery({
        ...
        queryFn: () =>
            apiGet<{ data: IssueRow[]; meta: PaginationMeta }>(
    ...
    const entryPointsQuery = useQuery({
        ...
        queryFn: () =>
            apiGet<{ data: EntryPoint[] }>("/admin/payments/entry-points").then(
                (res) => res.data,
            ),
        staleTime: 60000,
    });
    ...
    return (
        <div className="admin-linear-page admin-linear-payments space-y-8 text-foreground">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h1 className="mt-2 text-3xl font-semibold tracking-tight">
                        Operational visibility for ZengaPay and platform payment
                        flows
                    </h1>
                    <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
                        Watch successful, failed, pending, reversed, and
                        structurally risky flows in one place. This page also
                        maps which payment entry points are fully on the shared
                        ledger and which ones still bypass it.
                    </p>
                </div>
                <button
                    type="button"
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <StatCard
                    label="Failures"
                    value={String(
                        (summary?.failed ?? 0) + (summary?.reversed ?? 0),
                    )}
                    ...
                    <div className="flex flex-col gap-4 border-b border-border/70 pb-5 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                {paymentMeta
                                    ? `${paymentMeta.total.toLocaleString()} payments tracked in the ledger`
                                    : "Live payment rows"}
                            </p>
                        </div>
                            ...
                            <tbody className="divide-y divide-border/60">
                                {payments.length === 0 ? (
                                    <tr>
                                        ...
                                        >
                                            No payments matched the current
                                            filters.
                                        </td>
                                    </tr>
                                ) : (
                                            ...
                                            <td className="py-4 pr-4">
                                                ...
                                                {payment.latest_issue ? (
                                                    <div className="mt-2 inline-flex items-center gap-1 rounded-full bg-amber-100/80 px-2 py-1 text-[11px] font-medium text-amber-900 dark:bg-amber-950/70 dark:text-amber-200">
                                                        ...
                                                        {payment.latest_issue.issue_type.replace(
                                                            /_/g,
                                                            " ",
                                                        )}
                                                    </div>
                                            ...
                                            <td className="py-4">
                                                ...
                                                <div className="mt-1 text-xs text-muted-foreground">
                                                    {(() => {
                                                        const flowMinutes =
                                                            formatFlowMinutes(
                                                                payment.processing_age_minutes,
                                                            );
                    ...
                    <section className="rounded-3xl border border-border/70 bg-card/95 p-6 shadow-sm shadow-black/5 backdrop-blur-sm">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Payments that need investigation first
                                </p>
                            </div>
                        ...
                        <div className="mt-5 space-y-3">
                            {issues.length === 0 ? (
                                ...
                                issues.map((issue) => (
                                    <div
                                        key={issue.id}
                                        className="rounded-2xl border border-border/70 bg-background/70 p-4 shadow-sm shadow-black/5"
                                    >
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-600 dark:text-slate-300">
                                    Most recent operational warnings in the
                                    payment layer
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="mt-5 space-y-3">
                            {alerts.length === 0 ? (
                                ...
                                alerts.map((alert) => (
                                    <div
                                        key={alert.id}
                                        className="rounded-2xl border border-sky-200/70 bg-white/75 p-4 dark:border-white/10 dark:bg-white/5"
                                    >
                                        <div className="flex items-start justify-between gap-3">
                                            <div>
                                                <p className="font-medium">
            ...
            <section className="rounded-3xl border border-border/70 bg-card/95 p-6 shadow-sm shadow-black/5 backdrop-blur-sm">
                <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            This is the architecture view: which endpoints are
                            fully observable and which ones still bypass the
                            shared ledger.
                        </p>
                    </div>
                    <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                        Updated{" "}
                        {formatDate(dashboardQuery.data?.generated_at ?? null)}
                    </p>
                </div>
                ...
                <div className="mt-6 grid gap-4 xl:grid-cols-2">
                    {entryPoints.map((entryPoint) => (
                        <div
                            key={entryPoint.key}
                            className="rounded-2xl border border-border/70 bg-background/70 p-5 shadow-sm shadow-black/5"
                        >
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <StatusBadge
                                        status={entryPoint.integration_mode}
                                        size="sm"
                                        variant="info"
                                    />
                                    <StatusBadge
                            ...
                            <div className="mt-4 grid gap-3 sm:grid-cols-4">
                                ...
                                <div className="rounded-2xl border border-rose-200/60 bg-rose-50/80 p-3 dark:border-rose-900/60 dark:bg-rose-950/30">
                                    ...
                                    <p className="mt-2 text-2xl font-semibold text-rose-900 dark:text-rose-100">
                                        {entryPoint.metrics.open_issues}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="mt-4 grid gap-4 lg:grid-cols-3">
                                <div>
                                    ...
                                    <div className="mt-2 space-y-2">
                                        {entryPoint.initiation_endpoints.map(
                                            (endpoint) => (
                                                <code
                                                    key={endpoint}
                                                    className="block rounded-xl border border-border/70 bg-muted/45 px-3 py-2 text-xs text-foreground"
                                                >
                                                    {endpoint}
                                                </code>
                                            ),
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 space-y-2">
                                        {entryPoint.webhook_endpoints.length ===
                                        0 ? (
                                            <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                                                No webhook coverage
                                            </div>
                                        ) : (
                                            entryPoint.webhook_endpoints.map(
                                                (endpoint) => (
                                                    <code
                                                        key={endpoint}
                                                        className="block rounded-xl border border-border/70 bg-muted/45 px-3 py-2 text-xs text-foreground"
                            ...
                            <div className="mt-4 flex flex-wrap items-center gap-3">
                                <div className="rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground">
                                    Success rate:{" "}
                                    {entryPoint.success_rate === null
                                        ? "n/a"
                                        : `${entryPoint.success_rate}%`}
                                </div>
