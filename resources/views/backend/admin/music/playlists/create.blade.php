...
<div class="flex flex-col gap-4 sm:gap-6">
    ...
    <div class="flex items-center gap-x-2">
        ...
        <span class="text-slate-600 dark:text-navy-300">Create Playlist</span>
    </div>

    <!-- Page Header -->
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-slate-700 dark:text-navy-100">Create New Playlist</h1>
    </div>
    ...
    <div class="card">
        ...
        <form action="{{ route('admin.music.playlists.store') }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-5">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="privacy" class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent @error('privacy') border-red-500 @enderror">
                        ...
                        <option value="unlisted" {{ old('privacy') === 'unlisted' ? 'selected' : '' }}>Unlisted</option>
                    </select>
                    @error('privacy')
                        <span class="text-xs text-red-500 mt-1">{{ $message }}</span>
                    @enderror
            ...
            <div class="flex justify-end gap-2 pt-4">
                <a href="{{ route('admin.music.playlists.index') }}"
