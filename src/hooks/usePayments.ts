...
export function useWallet() {
  return useQuery({
    queryKey: ["wallet"],
    queryFn: () => apiGet<{ data: WalletInfo }>("/payments/wallet")
      .then(res => res.data),
...
export function useWalletTransactions(page: number = 1, perPage: number = 20) {
  return useQuery({
    ...
    queryFn: () => apiGet<WalletTransactionsResponse>("/payments/wallet/transactions", {
...
export function useWithdraw() {
  ...
  return useMutation({
    mutationFn: async (data: WithdrawRequest) => {
      const response = await apiPost<{ data: WithdrawResponse }>("/payments/wallet/withdraw", data);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["wallet"] });
