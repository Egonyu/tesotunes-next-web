...
<div class="space-y-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <span class="material-icons-round text-blue-600 dark:text-blue-400">edit</span>
                Edit Product: {{ $product->name }}
            </h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Edit product in {{ $store->name }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        <form action="{{ route('admin.store.products.update', [$store, $product]) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Price UGX -->
                    <div>
                        <label for="price_ugx" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Price (UGX) <span class="text-red-500">*</span>
                        </label>
                        <input type="number" name="price_ugx" id="price_ugx" value="{{ old('price_ugx', $product->price_ugx) }}" required min="0" step="0.01"
                               class="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:border-green-500 focus:ring-1 focus:ring-green-500"
                               placeholder="0.00">
                        @error('price_ugx')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="price_credits" id="price_credits" value="{{ old('price_credits', $product->price_credits) }}" min="0"
                               class="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:border-green-500 focus:ring-1 focus:ring-green-500"
                               placeholder="0">
                        @error('price_credits')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <!-- Compare at Price -->
                    <div>
                        <label for="compare_at_price_ugx" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Compare at Price (UGX)
                        </label>
                        <input type="number" name="compare_at_price_ugx" id="compare_at_price_ugx" value="{{ old('compare_at_price_ugx', $product->compare_at_price_ugx) }}" min="0" step="0.01"
                               class="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:border-green-500 focus:ring-1 focus:ring-green-500"
                               placeholder="0.00">
                        @error('compare_at_price_ugx')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="allow_credit_payment" id="allow_credit_payment" value="1" {{ old('allow_credit_payment', $product->allow_credit_payment) ? 'checked' : '' }}
                ...
                <!-- Allow Hybrid Payment -->
                <div class="flex items-center">
                    <input type="checkbox" name="allow_hybrid_payment" id="allow_hybrid_payment" value="1" {{ old('allow_hybrid_payment', $product->allow_hybrid_payment) ? 'checked' : '' }}
                           class="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="allow_hybrid_payment" class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                        Allow hybrid payment (credits + cash)
                    </label>
                </div>
            </div>
            ...
            <div class="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Stock Quantity -->
                    <div>
                        <label for="inventory_quantity" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Stock Quantity <span class="text-red-500">*</span>
                        </label>
                        <input type="number" name="inventory_quantity" id="inventory_quantity" value="{{ old('inventory_quantity', $product->inventory_quantity) }}" required min="0"
                               class="w-full px-3 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:border-green-500 focus:ring-1 focus:ring-green-500"
                               placeholder="0">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="allow_backorder" id="allow_backorder" value="1" {{ old('allow_backorder', $product->allow_backorder) ? 'checked' : '' }}
                               class="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 rounded focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        <label for="allow_backorder" class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                            Allow backorders
                        </label>
                    </div>
            ...
            <div class="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                @if($product->featured_image_url)
                    <div class="mb-4">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Current Featured Image:</p>
                        <img src="{{ $product->featured_image_url }}" alt="{{ $product->name }}" class="w-32 h-32 object-cover rounded-lg border border-gray-300 dark:border-gray-600">
                    </div>
                @endif
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm hover:shadow-md">
                    Update Product
                </button>
            </div>
        </form>
