    ...
    <div class="card mb-6">
        <div class="border-b border-slate-200 p-4 dark:border-navy-500">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('backend.music.artists.create') }}" class="btn bg-primary text-white hover:bg-primary-focus">
        ...
        <form method="GET" class="p-4">
            ...
            <div class="mt-4 flex justify-end space-x-2">
                <a href="{{ route('backend.music.artists.index') }}" class="btn border border-slate-300 font-medium text-slate-700 hover:bg-slate-150 dark:border-navy-450 dark:text-navy-100 dark:hover:bg-navy-500">
                    Clear
                </a>
                ...
                <tbody>
                    @forelse($artists ?? [] as $artist)
                        <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-400">
                                            @if($artist->genres->count() > 0)
                                                {{ $artist->genres->pluck('name')->implode(', ') }}
                                            @else
                                                No genres
                                            @endif
                                        </p>
                            ...
                            <td class="whitespace-nowrap px-4 py-3 lg:px-5">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('backend.music.artists.show', $artist) }}" class="btn size-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25">
                                    ...
                                    <a href="{{ route('backend.music.artists.edit', $artist) }}" class="btn size-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @if(!$artist->verified && $artist->verification_status === 'pending')
                                        <form method="POST" action="{{ route('backend.music.artists.verify', $artist) }}" class="inline">
                                            @csrf
                                    ...
                                    <form method="POST" action="{{ route('backend.music.artists.toggle-status', $artist) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="btn size-8 rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25"
