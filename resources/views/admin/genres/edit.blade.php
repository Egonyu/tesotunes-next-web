...
<div class="p-6 max-w-4xl mx-auto">
    <div class="mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-navy-50">Edit Genre</h1>
                <p class="mt-1 text-sm text-slate-600 dark:text-navy-300">Update {{ $genre->name }} information</p>
            </div>
        </div>
    </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" required value="{{ old('name', $genre->name) }}"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="form-input w-full rounded-lg border border-slate-300 dark:border-navy-450 bg-transparent px-3 py-2 placeholder:text-slate-400 hover:border-slate-400 focus:border-primary dark:hover:border-navy-400 dark:focus:border-accent"
                              placeholder="Brief description of this genre...">{{ old('description', $genre->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="color" name="color" id="color" value="{{ old('color', $genre->color ?? '#6366f1') }}"
                                   class="h-10 w-20 rounded border border-slate-300 dark:border-navy-450 cursor-pointer">
                            <input type="text" id="color_hex" value="{{ old('color', $genre->color ?? '#6366f1') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" id="icon" value="{{ old('icon', $genre->icon) }}"
                               class="form-input w-full rounded-lg border border-slate-300 dark:border-navy-450 bg-transparent px-3 py-2 placeholder:text-slate-400 hover:border-slate-400 focus:border-primary dark:hover:border-navy-400 dark:focus:border-accent"
                ...
                <div class="space-y-4">
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $genre->is_active) ? 'checked' : '' }}
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="is_featured" name="is_featured" type="checkbox" value="1" {{ old('is_featured', $genre->is_featured ?? false) ? 'checked' : '' }}
                                   class="form-checkbox size-5 rounded border-slate-300 dark:border-navy-450 bg-transparent checked:bg-primary checked:border-primary hover:border-slate-400 focus:border-primary dark:hover:border-navy-400 dark:checked:bg-accent dark:checked:border-accent dark:focus:border-accent">
                        </div>
                        <div class="ml-3">
